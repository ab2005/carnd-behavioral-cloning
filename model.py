...
def process_newimage_file(name):
    return imread('data/' + name) / 255. - 0.5
...
def trans_image(image,steer,trans_range):
    ...
    image_tr = cv2.warpAffine(image,Trans_M,(TRAIN_IMG_WIDTH,TRAIN_IMG_HEIGHT))
...
def preprocessImage(image):
    ...
    image = cv2.resize(image,(MODEL_IMG_WIDTH,MODEL_IMG_HEIGHT), interpolation=cv2.INTER_AREA)
    return image
...
def get_model():
    input_shape = (MODEL_IMG_HEIGHT, MODEL_IMG_WIDTH, 3)
    filter_size = 3
    pool_size = (2,2)
    model = Sequential()
